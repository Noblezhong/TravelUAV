#!/bin/bash
# pilot5 (0826) 40k hunger0.3+bw_symmetric0.2 模型 heldout 283 评估（todo#4 Step A 带宽决定 request）
root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root_dir"
source /home/noble/miniforge3/etc/profile.d/conda.sh && conda activate llamauav

WEIGHT=$root_dir/drl_train_pilot5_0826-2108/scheduler_models/ppo_scheduler_20260826-210837-103493.zip
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0827_pilot5_fast_x10
LOG=/tmp/pilot5_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot5 --gpu_id 0 \
  --simulator_tool_port 25000 --DDP_MASTER_PORT 80013 --batchSize 1 \
