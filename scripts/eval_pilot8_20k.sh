#!/bin/bash
# pilot8 (0901) 20k continuous+async 模型 heldout 283 评估（todo#4 判据v2）
...
WEIGHT=$(ls -dt $root_dir/drl_train_pilot8_20k_*/scheduler_models/ppo_scheduler_*.zip 2>/dev/null | head -1)
if [ -z "$WEIGHT" ]; then echo "ERROR: no pilot8 train output found (drl_train_pilot8_20k_*/scheduler_models/ppo_scheduler_*.zip)"; exit 1; fi
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0901_pilot8_20k_fast_x10
LOG=/tmp/pilot8_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot8 --gpu_id 0 \
