#!/bin/bash
# pilot4 (0825) 20k hunger+bw_symmetric 模型 heldout 283 评估（todo#4 rev3 buffer 饥饿 shaping）
root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root_dir"
source /home/noble/miniforge3/etc/profile.d/conda.sh && conda activate llamauav

WEIGHT=$root_dir/drl_train_pilot4_0825-1733/scheduler_models/ppo_scheduler_20260825-173325-512747.zip
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0825_pilot4_20k_fast_x10
LOG=/tmp/pilot4_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot4 --gpu_id 0 \
  --simulator_tool_port 25000 --DDP_MASTER_PORT 80012 --batchSize 1 \
