#!/bin/bash
# pilot7 (0828) 20k direct-conditioning 模型 heldout 283 评估（todo#4 判据v2）
# 判据: ① 高带宽 req 率 > 低带宽 ② 低 buffer req 率 > 高 buffer ③ 高 drift STOP 率 > 低; SR ≥25% 护栏
# 与训练传同样 shaping flags，reward_parts 记录与训练口径一致；scheduler_train_bw_iid 不传（eval 走真实 ucc4g trace）
root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root_dir"
source /home/noble/miniforge3/etc/profile.d/conda.sh && conda activate llamauav

WEIGHT=$(ls -dt $root_dir/drl_train_pilot7_20k_*/scheduler_models/ppo_scheduler_*.zip 2>/dev/null | head -1)
if [ -z "$WEIGHT" ]; then echo "ERROR: no pilot7 train output found (drl_train_pilot7_20k_*/scheduler_models/ppo_scheduler_*.zip)"; exit 1; fi
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0828_pilot7_20k_fast_x10
LOG=/tmp/pilot7_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot7 --gpu_id 0 \
  ...
  --scheduler_req_perfect_time_ms 5000.0 \
  --scheduler_req_bw_thresh_mbps 25.0 \
  --scheduler_req_buf_thresh 3.0 \
  --scheduler_req_shaping_pos 0.6 \
  --scheduler_req_shaping_buf 0.4 \
  --scheduler_req_shaping_inflight 0.4 \
  --scheduler_req_shaping_bw 0.4 \
  --scheduler_req_shaping_miss 0.4 \
  --scheduler_motion_drift_thresh_m 2.5 \
  --scheduler_motion_stop_good 1.0 \
  --scheduler_motion_stop_bad 0.8 \
  --scheduler_motion_cont_drift 0.6 \
