...
cd "${root_dir}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/vlnce_src/stopgo_ncn_eval.py \
  --run_type eval --name StopGoNCN --gpu_id 0 --simulator_tool_port "${SIMULATOR_PORT:-25000}" \
  --DDP_MASTER_PORT "${DDP_MASTER_PORT:-20032}" --batchSize 1 --always_help True --use_gt True \
  --maxWaypoints "${MAX_CONTROL_STEPS:-1000}" --max_episodes_per_scene 80 \
  --enable_comm_delay True --fast_eval True --fast_eval_speedup 10 --enable_ncn True \
  ...
  --eval_save_path "${EVAL_SAVE_PATH:-${root_dir}/eval_stopgo_ncn_${run_id}}" \
