#!/bin/bash
# pilot10 (0903) 20k continuous+async 模型 heldout 283 评估（todo#4 判据v2）
...
WEIGHT=$(ls -dt $root_dir/drl_train_pilot10_20k_*/scheduler_models/ppo_scheduler_*.zip 2>/dev/null | head -1)
if [ -z "$WEIGHT" ]; then echo "ERROR: no pilot10 train output found (drl_train_pilot10_20k_*/scheduler_models/ppo_scheduler_*.zip)"; exit 1; fi
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0903_pilot10_20k_fast_x10
LOG=/tmp/pilot10_eval20k.log
