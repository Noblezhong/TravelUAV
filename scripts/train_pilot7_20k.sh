#!/bin/bash
# pilot7 (0828) todo#4: direct-conditioning reward + 5-dim state + iid bandwidth, 20k train.
#   r_task:   −elapsed/5000 + (−5×illegal) + terminal{+40 success / +20 OSR / −20 coll / −10 fail}
#   r_req:    +0.6 req∧perfect∧bw_ok | −0.4 req∧buffer>3 | −0.4 req∧inflight | −0.4 req∧lowbw | −0.4 ¬req∧perfect∧bw_ok
#   r_motion: +1.0 stop∧drift≥2.5 | −0.8 stop∧drift<2.5 | −0.6 ¬stop∧drift≥2.5
#   state = [buffer/7, bw/1e8, inflight, time_drift/5000, drift/2.5]  (5-dim, MlpPolicy)
root_dir=.
...
CUDA_VISIBLE_DEVICES=0 python -u $root_dir/src/vlnce_src/drl_scheduler_train.py \
    --run_type eval \
    --name TravelLLMPCDRLSchedulerTrainPilot7 \
    --gpu_id 0 \
    ...
    --eval_save_path $REPO_ROOT/drl_train_pilot7_20k_$run_ts \
