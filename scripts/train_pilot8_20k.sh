#!/bin/bash
# pilot8 (0901) todo#4: continuous reward + async request, 20k train.
...
CUDA_VISIBLE_DEVICES=0 python -u $root_dir/src/vlnce_src/drl_scheduler_train.py \
    --run_type eval \
    --name TravelLLMPCDRLSchedulerTrainPilot8 \
    --gpu_id 0 \
    ...
    --eval_save_path $REPO_ROOT/drl_train_pilot8_20k_$run_ts \
