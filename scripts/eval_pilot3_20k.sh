#!/bin/bash
# pilot3 (0825) 20k bw_symmetric 模型 heldout 283 评估（todo#4 rev2 带宽对称激励）
root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root_dir"
source /home/noble/miniforge3/etc/profile.d/conda.sh && conda activate llamauav

WEIGHT=$root_dir/drl_train_pilot3_0825-1013/scheduler_models/ppo_scheduler_20260825-101348-054182.zip
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0825_pilot3_20k_fast_x10
LOG=/tmp/pilot3_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot3 --gpu_id 0 \
  --simulator_tool_port 25000 --DDP_MASTER_PORT 80011 --batchSize 1 \
