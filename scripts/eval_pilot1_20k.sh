#!/bin/bash
# pilot1 (0821) 20k 模型 heldout 283 评估
root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root_dir"
source /home/noble/miniforge3/etc/profile.d/conda.sh && conda activate llamauav

WEIGHT=$root_dir/drl_train_pilot1_0821-1822/scheduler_models/ppo_scheduler_20260821-223507-441523.zip
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0821_pilot1_20k_fast_x10
LOG=/tmp/pilot1_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot1 --gpu_id 0 \
