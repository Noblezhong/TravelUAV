#!/bin/bash
# pilot2 (0824) 20k bw_scaled 模型 heldout 283 评估（todo#4 带宽自适应）
root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root_dir"
source /home/noble/miniforge3/etc/profile.d/conda.sh && conda activate llamauav

WEIGHT=$root_dir/drl_train_pilot2_0824-1133/scheduler_models/ppo_scheduler_20260824-113345-134738.zip
EVALJSON=$root_dir/heldout_283_evalformat.json
SAVE=$root_dir/eval_drl_0824_pilot2_20k_fast_x10
LOG=/tmp/pilot2_eval20k.log

echo "model: $WEIGHT"
CUDA_VISIBLE_DEVICES=0 python -u "$root_dir/src/vlnce_src/drl_scheduler_eval.py" \
  --run_type eval --name TravelLLMPCDRLSchedulerEvalPilot2 --gpu_id 0 \
  --simulator_tool_port 25000 --DDP_MASTER_PORT 80010 --batchSize 1 \
