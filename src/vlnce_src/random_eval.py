...
class RuleBasedScheduler:
    ...
    def decide(
        ...
        if active_state_drift_m >= self.request_drift_m:
            return SchedulerDecision(
...
class RandomScheduler:
    ...
    (STOP/CONTINUE x REQUEST/NO_REQUEST) uniformly at random, over the
    legal action set at the current state.  STOP_ONLY is only legal while
    a request is in flight (planner_has_inflight=True): its handler waits
    on planner.wait_result(), which has nothing to wait for otherwise and
    would block forever.  Interpreting a choose of CONTINUE_ONLY when the
    waypoint buffer is already drained is absorbed by the eval loop (the
    `if active_index < len(active_traj)` guard skips execution; the next
    draw that yields a request/stop action restores progress).
    """
    ...
    def decide(
        ...
        if planner_has_inflight:
            actions = (STOP_AND_REQUEST, STOP_ONLY, CONTINUE_AND_REQUEST, CONTINUE_ONLY)
        else:
            actions = (STOP_AND_REQUEST, CONTINUE_AND_REQUEST, CONTINUE_ONLY)
...
def eval(
    ...
    with torch.no_grad():
        ...
        with open(profile_log_path, "w", encoding="utf-8") as profile_fp:
            episode_idx = 0
            scheduler = RandomScheduler(seed=int(args.random_seed))
            while True:
