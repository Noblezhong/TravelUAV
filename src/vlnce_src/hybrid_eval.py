...
CONTINUE_NO_REQUEST = "CONTINUE_NO_REQUEST"
STOP_NO_REQUEST = "STOP_NO_REQUEST"
...
class HybridScheduler:
    """Fixed-threshold rule-based scheduler, as specified in Algorithm 1 of the paper.

    Uses only buffer occupancy b_m, state shift Delta_m, and request status f_m
    (whether a request is in flight).  No oracle distance, no bandwidth/time
    observation.  When a request is already in flight no new request is issued;
    the UAV stops and waits for the in-flight result only when the buffer is
    empty or drift exceeds the stop threshold.
    """
    ...
    def decide(
        ...
        # f_m = 1: a request is in flight, never issue another one.
        if planner_has_inflight:
            if buffer_remaining <= 0 or active_state_drift_m >= self.stop_drift_m:
                return SchedulerDecision(
                    STOP_NO_REQUEST,
                    min(1.0, active_state_drift_m / self.stop_drift_m),
                    ["buffer_empty", f"state_drift>={self.stop_drift_m:.1f}m"],
                )
            return SchedulerDecision(CONTINUE_NO_REQUEST, 0.0, ["planner_inflight"])

        # f_m = 0: request when the buffer is empty or drift is large.
        if buffer_remaining <= 0 or active_state_drift_m >= self.stop_drift_m:
            return SchedulerDecision(
                STOP_AND_REQUEST,
                min(1.0, active_state_drift_m / self.stop_drift_m),
        ...
        if buffer_remaining <= self.request_buffer_remaining or active_state_drift_m >= self.request_drift_m:
            return SchedulerDecision(
                CONTINUE_AND_REQUEST,
                0.7,
                [f"buffer_remaining<={self.request_buffer_remaining}", f"state_drift>={self.request_drift_m:.1f}m"],
            )
...
def _print_exec_profile_line(episode_idx, step_idx, profile_info, current_chunk=None, current_pose=None):
    line = (
        ...
        f"scheduler={profile_info.get('scheduler_action', 'None')} "
...
def _distance_trend(distance_to_ends: List[float], window: int = 5):
    if len(distance_to_ends) < 2:
        return None
    tail = distance_to_ends[-window:]
    if len(tail) < 2:
        return None
...
def _active_state_drift_m(active_result: Optional[PlannerResult], current_pose):
    if active_result is None:
        return 0.0
    return float(math.dist(active_result.observation_pose, current_pose))


def _planner_has_inflight_without_ready(planner: "LatestOnlyEdgePlanner"):
...
def _add_hybrid_fields(
    ...
    *,
    ...
    buffer_remaining: int,
    planner_has_inflight: bool,
    active_result: Optional[PlannerResult],
    current_pose,
    ...
    record.update(
        {
            ...
            "request_decision": bool(request_decision),
            "buffer_remaining": int(buffer_remaining),
            "planner_has_inflight": bool(planner_has_inflight),
            "active_request_id": int(active_result.request_id) if active_result is not None else None,
            "active_action_delay_ms": active_action_delay_ms,
            "active_state_drift_m": float(active_state_drift_m),
            "risk_score": float(decision.risk_score) if decision is not None else None,
            "risk_reasons": copy.deepcopy(decision.risk_reasons) if decision is not None else [],
            "mode_switch": bool(action is not None and previous_scheduler_action is not None and action != previous_scheduler_action),
            "stop_wait_ms": float(stop_wait_ms),
...
def eval(
            ...
            while True:
                ...
                for retry_i in range(3):
                    ...
                    try:
                        ...
                        warmup_decision = SchedulerDecision(STOP_AND_REQUEST, 1.0, ["cold_start"])
                        ...
                        _add_hybrid_fields(
                            ...
                            buffer_remaining=len(active_traj),
                        ...
                        while not state.dones[0] and control_step < int(args.max_control_steps):
                            ...
                            if active_index < len(active_traj):
                                executed_traj_snapshot = copy.deepcopy(active_traj)
                                executed_active_result = active_result
                                ...
                                elif scheduler_decision.action == STOP_NO_REQUEST and not state.dones[0]:
                                    # (STOP, NO_REQUEST): stop and wait for the in-flight
                                    # result; do not issue a new request.
                                    hover_wait_start = time.perf_counter()
                                    hover_logical_start = episode_clock.now_ms
                                    if planner.has_inflight() and not state.dones[0]:
                                        ...
                                        trajectory_switch_applied = True
                                        current_pose = state.current_sim_pose()
                                        stop_decision_record = _build_planner_decision_record(
                                            env_batchs[0],
                                            applied_result,
                                            current_pose,
                                            enable_comm_delay,
                                        ...
                                        stop_decision_record["predict_dones"] = [bool(x) for x in state.predict_dones]
                                        stop_decision_record["collisions"] = [bool(x) for x in state.collisions]
                                        stop_decision_record["dones"] = [bool(x) for x in state.dones]
                                        stop_decision_record["stale_result_discarded"] = False
                                        _add_hybrid_fields(
                                        ...
                                        _print_decision_profile_line(episode_idx, applied_result.request_id, stop_decision_record)
                                        _print_trajectory_bundle(
                                            episode_idx,
                                            applied_result.request_id,
                                            applied_result.llm_output,
                                ...
                                record = {
                                    ...
                                    "comm_delay_enabled": bool(enable_comm_delay),
                                    "request_id": int(executed_active_result.request_id) if executed_active_result is not None else None,
                                    "request_submitted_step": int(executed_active_result.submitted_step) if executed_active_result is not None else None,
                                    "request_observation_timestamp": int(executed_active_result.observation_timestamp) if executed_active_result is not None else None,
                                    "request_ready_wall_time": float(executed_active_result.ready_wall_time) if executed_active_result is not None else None,
                                    ...
                                    "uplink_payload_bits": int(executed_active_result.uplink_payload_bits) if executed_active_result is not None else None,
                                    ...
                                    "llm_output": None,
                                    "refined_waypoints": copy.deepcopy(executed_traj_snapshot),
                                    "current_pose": state.current_sim_pose(),
                                    "first_refined_gap_m": float(_point_delta(executed_traj_snapshot[0], state.current_sim_pose())) if len(executed_traj_snapshot) > 0 else None,
                                ...
                                _add_hybrid_fields(
                                    record,
                                    scheduler_decision,
                                    execution_mode="stop_and_go" if scheduler_decision.action in (STOP_AND_REQUEST, STOP_NO_REQUEST) else "continuous",
                                    request_decision=scheduler_decision.action in (STOP_AND_REQUEST, CONTINUE_AND_REQUEST),
                                    buffer_remaining=buffer_remaining,
                                    planner_has_inflight=_planner_has_inflight_without_ready(planner),
                                    active_result=executed_active_result,
                                    current_pose=state.current_sim_pose(),
                                    state=state,
                                    previous_scheduler_action=previous_scheduler_action,
                                    stop_wait_ms=hover_wait_ms,
                                ...
                                previous_scheduler_action = scheduler_decision.action
    ...
    scheduler_records = [
        ...
        if item.get("scheduler_countable") and item.get("scheduler_action") in (STOP_AND_REQUEST, CONTINUE_AND_REQUEST, CONTINUE_NO_REQUEST, STOP_NO_REQUEST)
    ]
    scheduler_action_counts = {
        action: sum(1 for item in scheduler_records if item.get("scheduler_action") == action)
        for action in (STOP_AND_REQUEST, CONTINUE_AND_REQUEST, CONTINUE_NO_REQUEST, STOP_NO_REQUEST)
    }
    scheduler_total = sum(scheduler_action_counts.values())
    scheduler_action_ratio = {
        action: (count / scheduler_total if scheduler_total > 0 else 0.0)
        for action, count in scheduler_action_counts.items()
    }
    summary = {
        ...
        "scheduler_action_counts": scheduler_action_counts,
